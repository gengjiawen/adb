...
Note that when the Pairing client finishes on the host, it also automatically attempts to connect to the device 
is just paired with. This is because `_adb-tls-connect` was already published before pairing even began, which
means the host cannot rely on mDNS `_adb-tls-connect` "Create" being published.
...
- `persist.adb.tls_server.enable`: Set when Settings UI checkbox "Use wireless debugging" is changed.
adbd listen for this value and manages the TLSServer lifecycle publishing accordingly.
