...
std::optional<ProcessInfo> readProcessInfoFromSocket(int socket) {
  ...
  int rc = TEMP_FAILURE_RETRY(recv(socket, proto.data(), proto.length(), MSG_PEEK));
  if (rc <= 0) {
    LOG(ERROR) << "adbconnection_server: Unable to MSG_PEEK ProcessInfo recv buffer" << rc;
    return {};
  }

  uint32_t message_size = rc;
    ...
    for (int i = 0; i < epoll_rc; ++i) {
      ...
      if (event.data.fd == -1) {
        ...
        if (process_info) {
          ...
          PLOG(ERROR) << "Unable to read ProcessInfo from app startup";
        }
