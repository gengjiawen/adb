...
└──┘ read(3)  └─────────────────┼─────────────────┼────────────┘
    ...
    Note that a socket can be peered regardless of their kind. A remote socket can be peered with
    a smart socket, a local socket can be peered with a remote socket an so on.
 */
struct asocket {
     ...
     * TODO(sanglardf): This should be a boolean.
     */
    int closing = 0;
