...
static bool handle_send_file(borrowed_fd s, const char* path, uint32_t* timestamp, uid_t uid,
    ...
    if (!dry_run) {
        ...
        if (fd < 0) {
            ...
            if (getuid() != 0) {
                SendSyncFail(s, StringPrintf("skipping fchown() uid: %d gid: %d", uid, gid));
                goto fail;
            }

            else if (fchown(fd.get(), uid, gid) == -1) {
                struct stat st;
                std::string real_path;

                // Return failure if parent directory does not have S_ISGID bit set,
                ...
                if (!Realpath(path, &real_path) || lstat(Dirname(real_path).c_str(), &st) == -1 ||
                    ...
                    SendSyncFail(s, StringPrintf("fchown() failed for uid: %d gid: %d", uid, gid));
                    goto fail;
                }
            }
