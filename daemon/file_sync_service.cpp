...
void handle_error(borrowed_fd s, uid_t uid, gid_t gid) {
    const std::string msg(StringPrintf("fchown() failed for uid: %d gid: %d", uid, gid));
    LOG(WARNING) << msg;
    SendSyncFailErrno(s, msg);
}
