...
bool PropertyMonitor::RunOnce() {
    // First, call our callbacks for newly-watched properties.
    ...
    for (auto& [property_name, data] : properties_) {
        ...
        if (FindProperty(property_name, &data)) {
            ...
            result &= data.callback(std::string());
        }
    ...
    while (!should_exit) {
        // Then, wait for properties to change.
        current_serial = WaitForSerialChange(last_serial_);
        last_serial_ = current_serial;
        for (auto& [property_name, data] : properties_) {
            if (!data.prop_info) {
                if (FindProperty(property_name, &data)) {
