...
static void jdwp_process_event(int socket, unsigned events, void* _proc) {
    ...
    if (events & FDE_READ) {
        ...
        int rc = TEMP_FAILURE_RETRY(recv(socket, &message_size, sizeof(uint32_t), 0));

        // The app has terminated and the socket was closed.
        if (rc <= 0) {
        ...
        proc->process = readProcessInfoFromSocket(socket, message_size);
...
static void process_list_updated(TrackerKind kind) {
    ...
    std::string data;
    // There is no reason to limit the size!
    // Let's just bump the value to 1MiB for now to keep this topic volume of code low
    // and remove this limitation in the next CL.
    const int kMaxLength = kind == TrackerKind::kJdwp ? 1024 : 10 * 2048;
