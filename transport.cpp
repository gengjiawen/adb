...
static void transport_registration_func(int _fd, unsigned ev, void*) {
    LOG(INFO) << ": transport_registration_func";

    tmsg m;
    atransport* t;

    if (!(ev & FDE_READ)) {
        LOG(INFO) << ": return, not env";
        return;
    }

    if (transport_read_action(_fd, &m)) {
        LOG(INFO) << ": cannot read transport registration socket";
    ...
    if (m.action == tmsg::Action::UNREGISTER) {
        LOG(INFO) << ": transport_registration_func tmsg::Action::UNREGISTER";
    ...
    if (t->GetConnectionState() != kCsNoPerm) {
        LOG(INFO) << ": transport_registration_func GetConnectionState() != kCsNoPerm";
        ...
        if (t->type == kTransportUsb
            ...
            LOG(INFO) << ": transport_registration_func before Start()";
            t->connection()->Start();
#if ADB_HOST
            send_connect(t);
#endif
    ...
    LOG(INFO) << ": transport_registration_func before update_transports()";
    update_transports();
}
...
void init_transport_registration(void) {
    LOG(INFO) << ": init_transport_registration";
    int s[2];
