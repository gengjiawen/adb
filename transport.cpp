...
//
// FOLLOW-UP(sanglardf): Is this fdevent bridge really necessary? With the re-entrant lock to sync
// access, what prevents us from "simply" using the transport_list directly?

struct tmsg {
    atransport* transport;

    enum struct Action : int {
        REMOVE = 0,    // Remove the transport from transport list.
        REGISTER = 1,  // Add the transport to the transport list.
    };
    Action action;
};
...
static void transport_registration_func(int _fd, unsigned ev, void*) {
    ...
    if (m.action == tmsg::Action::REMOVE) {
