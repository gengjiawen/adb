...
struct device_tracker {
    ...
    TrackerOutputType output_type = TEXT;
...
static std::string transportListToProto(const std::list<atransport*>& sorted_transport_list,
    ...
    for (const auto& t : sorted_transport_list) {
        ...
        device->set_devpath(sanitize(t->devpath, false));
        ...
        device->set_max_speed(t->connection()->MaxSpeedMbs());
        device->set_negotiated_speed(t->connection()->NegotiatedSpeedMbs());
    }

    // TODO Pick binary or text proto output
    if (text_version) {
        std::string string_proto;
        google::protobuf::TextFormat::PrintToString(devices, &string_proto);
        return string_proto;
    } else {
        std::string result;
        devices.SerializeToString(&result);
        return result;
    }
}
...
std::string list_transports(TrackerOutputType outputType) {
    ...
    switch (outputType) {
        case TEXT:
        case LONG_TEXT: {
            return transportListToText(sorted_transport_list, outputType == TEXT);
        }
        case PROTOBUFFER:
        case TEXT_PROTOBUFFER: {
            return transportListToProto(sorted_transport_list, outputType == TEXT_PROTOBUFFER);
        }
        default: {
            return "";
        }
