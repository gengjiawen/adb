...
TEST_F(FdeventTest, smoke) {
    for (bool use_new_callback : {true, false}) {
        ...
        fdevent_run_on_looper_thread([&thread_arg, &fd_handlers, use_new_callback]() {
        ...
        fdevent_run_on_looper_thread([&fd_handlers]() { fd_handlers.clear(); });
        WaitForFdeventLoop();
...
TEST_F(FdeventTest, run_on_looper_thread) {
    ...
    fdevent_run_on_looper_thread([]() {
        check_looper_thread();
        std::this_thread::sleep_for(std::chrono::seconds(1));
    });

    for (int i = 0; i < 1000000; ++i) {
        fdevent_run_on_looper_thread([i, &vec]() {
            check_looper_thread();
            vec.push_back(i);
        });
    }
...
// Initial invocation is on a non-looper thread, whereas subsequent
// invocations execute on the looper thread.
static std::function<void()> make_appender(std::vector<int>* vec, int invocation_id) {
    ...
    return [vec, invocation_id]() {
        ...
        fdevent_run_on_looper_thread(make_appender(vec, invocation_id + 1));

        CHECK_LT(invocation_id, 100);
    };
}

// Internal pre-conditional assert function that enforces a strong guarantee
// (applicable only to test).
...
TEST_F(FdeventTest, run_on_looper_thread_reentrant) {
    ...
    PrepareThread();
    fdevent_run_on_looper_thread(make_appender(&vec, 0));
    TerminateThread();
    AssertMainThread();
