...
struct fdevent_context {
  ...
  public:
    ...
    // Assert that the caller is executing on the event-thread (thread which
    // invoked Loop(), the pseudo main thread), or that there is no active
    // main (pseudo) thread.
    void CheckLooperThread() const;

    // If there is an active pseudo-main thread, assert that the invocation
    // is *not* on that context.
    void CheckNotLooperThread() const;

    // Queue an operation to be run on the main/eveant thread.
