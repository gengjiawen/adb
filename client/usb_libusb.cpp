...
static std::unordered_map<uint64_t, libusb_device*> known_devices
        [[clang::no_destroy]] GUARDED_BY(known_devices_lock);
...
// immediately.
static uint64_t get_key_for(libusb_device* dev) {
    ...
    //    LOG(DEBUG) << "Generated key " << key << "for vendor=" << vendor << " prod=" << product
...
struct LibusbConnection : public Connection {
    struct ReadBlock {
        ...
        // True is the block has been sent in a libusb transfer
        bool active = false;
    };
    ...
    void Cleanup(ReadBlock* read_block) REQUIRES(read_mutex_) {
        // When windows go to sleep it powers off all USB Host controller in order to save
        // energy . When Windows wakes up, it powers them up which cause all the endpoints
        // to reset (which generate a read failure leading to use Close()ing the device). The
        // USB device briefly goes away and comes back with the exact same properties. This
        // makes in-house hot-plug miss devices reconnection upon wakeup. To solve that problems, we
        // remove ourselves from the list of know devices when read fails.
        remove_from_inhouse_hotplug(read_block->self);
    ...
    static void remove_from_inhouse_hotplug(const LibusbConnection* self) {
        if (false && libusb_has_capability(LIBUSB_CAP_HAS_HOTPLUG)) {
            return;
        }
        std::lock_guard<std::mutex> lock(known_devices_lock);
        auto key = get_key_for(self->device_.get());
        known_devices.erase(key);
        VLOG(USB) << "Removing self =" << key << " from known devices";
    }
    ...
    void CloseDevice() {
        ...
        // Terminate all write operations.
        {
    ...
    // asocket protocol is made of a header amessage optionally followed by a payload.
    // We constantly have a read transfer pending in libusb to read the amessage header. When
    // that transfer callback trigger we either send a payload transfer read request
    // or a header transfer read request in case the amessage has no payload.
    //
    // This design makes us detect device disconnections before the hotplug callback reports them
    // because the header transfer callback triggers with a failure code.
    ...
    // We keep track of all pending transfers submitted to libusb. This allows to cancel them if we
    // Close the Device. Transfers are also removed from this map when libusb transfer callback
    // triggers. Each transfer submission is tagged with a unique sequential id.
    ...
    // Close()ing a device involves waiting for libusb transfer write/read callbacks to trigger on
    // the libusb event thread. We use a cv to wait until these callbacks trigger.
...
static void inhouse_hotplug_scan() {
    ...
    //    VLOG(USB) << "Known devices #=" << known_devices.size();
    ...
    //    VLOG(USB) << "Current devices before #=" << current_devices.size();
    //    for (const auto& [key, dev] : current_devices) {
    //        VLOG(USB) << key;
    //    }
    known_devices = std::move(current_devices);
    //    VLOG(USB) << "Known devices after #=" << known_devices.size();
    //    for (const auto& [key, dev] : known_devices) {
    //        VLOG(USB) << key;
    //    }
