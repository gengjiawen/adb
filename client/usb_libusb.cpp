...
struct LibusbConnection : public Connection {
    ...
    bool OpenDevice(std::string* error) {
        ...
        if (rc != 0) {
            VLOG(USB) << "failed to claim adb interface on " << device_address_;
            LOG_ERR(error, "failed to claim adb interface for device serial='%s': %s",
                    serial_.c_str(), libusb_error_name(rc));
            device_handle_.reset();
            return false;
        }
...
// How long to wait between in-house scan
static auto inhouse_scan_delay_s = 2;
// Last time in-house hotplug canned devices
static auto last_inhouse_scan = std::chrono::steady_clock::now();
static void inhouse_hotplug_scan() {
    //    return;
    // This is called on libusb even thread. We only scan periodically.
    auto now = std::chrono::steady_clock::now();
    auto time_since_last_run =
            std::chrono::duration_cast<std::chrono::seconds>(now - last_inhouse_scan).count();
    if (time_since_last_run < inhouse_scan_delay_s) {
        return;
    }
    last_inhouse_scan = std::chrono::steady_clock::now();
