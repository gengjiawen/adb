...
struct LibusbConnection : public Connection {
    ...
    void RetrieveSpeeds() {
        negotiated_spped_ = ToConnectionSpeed(libusb_get_device_speed(device_.get()));

        // The set of supported speed is in a SuperSpeed capability
        struct libusb_bos_descriptor* bos = nullptr;
        int ret = libusb_get_bos_descriptor(device_handle_.get(), &bos);
        if (!ret) {
    ...
    virtual ConnectionSpeed MaxSpeedMbs() override final { return max_speed_; }

    virtual ConnectionSpeed NegotiatedSpeedMbs() override final { return negotiated_spped_; }
    ...
    ConnectionSpeed negotiated_spped_ = UNKNOWN;
