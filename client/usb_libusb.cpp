...
struct LibusbConnection : public Connection {
    ...
    std::string GetUsbDeviceAddress() { return std::string("usb:") + device_address_; }
    ...
    static std::optional<std::shared_ptr<LibusbConnection>> Create(unique_device device) {
        ...
        if (!device_desc) {
            LOG(INFO) << "ignoring device: not an adb interface. (GetDeviceDescriptor)";
            return {};
        }

        if (!connection->FindInterface(&device_desc.value())) {
            LOG(INFO) << "ignoring device: not an adb interface. (FindInterface)";
            return {};
        }
        ...
        if (!connection->OpenDevice(nullptr)) {
            LOG(INFO) << "ignoring device: not an adb interface. (OpenDevice)";
            return {};
        }
        ...
        if (!transport_server_owns_device(connection->GetUsbDeviceAddress(), connection->serial_)) {
            LOG(INFO) << "ignoring device: not owned by this server dev_path: '"
                      << connection->GetUsbDeviceAddress() << "' serial: '" << connection->serial_
                      << "'";
            return {};
        }
