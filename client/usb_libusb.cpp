...
// For in-house hotplug, we maintain a list of last known devices.
static std::mutex& in_house_hotplug_known_devices_lock = *new std::mutex();
static std::unordered_map<uint64_t, libusb_device*> in_house_hotplug_known_devices
        [[clang::no_destroy]]
        GUARDED_BY(in_house_hotplug_known_devices_lock);
// For in-house hotplug, we generate an unique identified based on device invariant (vendor,
// product), the USB port, and the address (the location in the USB chain). On Windows,
// the address is always incremented, even if the same device is unplugged and plugged immediately.
...
struct LibusbConnection : public Connection {
    ...
    // When a Windows machine go to sleep it powers off all its USB Host controller to save
    // energy. When the machines awaken, it powers them up which cause all the endpoints
    // to be closed (which generate a read/write failure leading to us Close()ing the device). The
    ...
    // problem, we remove ourselves from the list of know devices when read/write fails.
    static void remove_from_inhouse_hotplug(uint64_t key) {
        if (libusb_has_capability(LIBUSB_CAP_HAS_HOTPLUG)) {
            return;
        }
        std::lock_guard<std::mutex> lock(in_house_hotplug_known_devices_lock);
...
static void inhouse_hotplug_scan() {
    ...
    for (size_t i = 0; devs[i] != nullptr; i++) {
        ...
        if (in_house_hotplug_known_devices.count(key) == 0) {
