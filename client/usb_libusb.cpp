...
struct LibusbConnection : public Connection {
    ...
    void CancelReadTransfer(ReadBlock* read_block) REQUIRES(read_mutex_) {
        ...
        if (read_block->active) {
            ...
            if (rc == 0) {
                // This will be cleaned up by read_cb.
                return;
            } else {
                LOG(INFO) << "libusb_cancel_transfer header_read_ failed: "
                          << libusb_error_name(rc);
            }
        }
    ...
    void CloseDevice() {
        ...
        if (device_handle_) {
            ...
            if (rc != 0) {
                LOG(INFO) << "libusb_cancel_transfer payload_read_ failed: "
                          << libusb_error_name(rc);
            }
            device_handle_.reset();
        }
...
// this adb server is allowed to speak to. This controls what devices an adb
// server can interact with.
// This prevents any interaction with the device and makes it invisible on
// "adb devices".
// example: ADB_LIBUSB_FILTER=2-5,2-12.1
