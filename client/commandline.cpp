...
static void help() {
    ...
    fprintf(stdout,
        ...
        "\n"
        "general commands:\n"
        " devices [-l][-tp]        list connected devices (-l for long output, -pt text protobuffer output)\n"
...
int adb_commandline(int argc, const char** argv) {
    ...
    if (!strcmp(argv[0], "devices")) {
        ...
        if (argc < 2) {
            listopt = "";
        } else if (argc == 2 && (!strcmp(argv[1], "-l") || !strcmp(argv[1], "-tp"))) {
            listopt = argv[1];
        } else {
            error_exit("adb devices [-l][-tp]");
        }
    ...
    /* other commands */
        ...
        if (argc < 2) {
            listopt = "";
                   ...
                   (!strcmp(argv[1], "-l") && !strcmp(argv[1], "-tp") && !strcmp(argv[1], "-p"))) {
            error_exit("usage: adb track-devices [-l][-p][-tp]");
        } else {
            listopt = argv[1];
        }
        std::string query = android::base::StringPrintf("host:%s%s", argv[0], listopt);
        ...
        //        }
        //        if (!strcmp(argv[1], "-l")) {
        //            return adb_connect_command("host:track-devices-l");
        //        }
        //        return adb_connect_command("host:track-devices-tp");
