...
static void help() {
    ...
    fprintf(stdout,
        ...
        "     run remote shell command (interactive shell if no command given). If supported\n"
        "     differentiate stdin, stderr, and retrieve exit code (shell,v2).\n"
        "     -e: choose escape character, or \"none\"; default '~'\n"
        "     -n: don't read from stdin\n"
        "     -T: disable pty allocation\n"
        "     -t: allocate a pty if on a tty (-tt: force pty allocation)\n"
        "     -x: disable exit codes and stdout/stderr separation (shell,v1)\n"
