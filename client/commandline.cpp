...
static void help() {
    ...
    fprintf(stdout,
        ...
        "       <host>:[<port>]               -> The same as tcp:<host>:[<port>]\n"
        "       tcp:<host>:[<port>]           -> TCP/IP [default port=5555]\n"
        "       vsock:<CID>:<port>            -> vsock\n"
        "       localfilesystem:<path>        -> Named UNIX domain socket at the specified path\n"
        "       emu:<console port>,<adb port> -> Connect to the specified emulator\n"
        "       command:<command>             -> Connect using the specified command\n"
        "           ADB will execute the command and perform IO through the stdin and\n"
        "           stdout of the command process\n"
        " disconnect [REMOTE]\n"
        "     disconnect from given device, or all\n"
