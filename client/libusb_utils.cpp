...
void LibusbConnection::HandlePacket(amessage& msg, std::optional<Block> payload) {
...
void LibusbConnection::Cleanup(ReadBlock* read_block) REQUIRES(read_mutex_) {
...
bool LibusbConnection::MaybeCleanup(ReadBlock* read_block) REQUIRES(read_mutex_) {
    CHECK(read_block);
...
void LIBUSB_CALL LibusbConnection::header_read_cb(libusb_transfer* transfer) {
    ...
    if (transfer->actual_length != sizeof(amessage)) {
        std::string msg =
                StringPrintf("usb read: invalid length for header: %d", transfer->actual_length);
        LOG(ERROR) << msg;
        self->OnError(msg);
        self->Cleanup(read_block);
        return;
    ...
    if (amsg.data_length > MAX_PAYLOAD) {
        std::string msg = StringPrintf("usb read: payload length too long: %d", amsg.data_length);
        LOG(ERROR) << msg;
        self->OnError(msg);
...
void LIBUSB_CALL LibusbConnection::payload_read_cb(libusb_transfer* transfer) {
    auto read_block = static_cast<ReadBlock*>(transfer->user_data);
    auto self = read_block->self;
    std::lock_guard<std::mutex> lock(self->read_mutex_);
    ...
    if (transfer->actual_length != transfer->length) {
        std::string msg = StringPrintf("usb read: unexpected length for payload: wanted %d, got %d",
...
void LIBUSB_CALL LibusbConnection::write_cb(libusb_transfer* transfer) {
...
bool LibusbConnection::DoTlsHandshake(RSA*, std::string*) {
    LOG(FATAL) << "tls not supported";
    return false;
}

void LibusbConnection::CreateRead(ReadBlock* read, bool header) {
    read->self = this;
...
void LibusbConnection::SubmitRead(ReadBlock* read, size_t length) {
...
void LibusbConnection::SubmitWrite(Block&& block) REQUIRES(write_mutex_) {
...
std::optional<libusb_device_descriptor> LibusbConnection::GetDeviceDescriptor() {
...
bool LibusbConnection::FindInterface(libusb_device_descriptor* device_desc) {
...
std::string LibusbConnection::GetUsbDeviceAddress() const {
    return std::string("usb:") + device_address_;
}

std::string LibusbConnection::GetSerial() {
    std::string serial;

    auto device_desc = GetDeviceDescriptor();

    serial.resize(255);
    int rc = libusb_get_string_descriptor_ascii(device_handle_.get(), device_desc->iSerialNumber,
...
bool LibusbConnection::OpenDevice(std::string* error) {
    if (device_handle_) {
    ...
    if (rc != 0) {
        LOG_ERR(error, "failed to claim adb interface for device '%s': %s", serial().c_str(),
                libusb_error_name(rc));
        device_handle_.reset();
        return false;
    }
    ...
    for (uint8_t endpoint : {read_endpoint_, write_endpoint_}) {
        ...
        if (rc != 0) {
            LOG_ERR(error, "failed to clear halt on device '%s' endpoint %#02x: %s",
                    serial().c_str(), endpoint, libusb_error_name(rc));
...
void LibusbConnection::CancelReadTransfer(ReadBlock* read_block) REQUIRES(read_mutex_) {
...
void LibusbConnection::CloseDevice() {
    // This is rather messy, because of the lifecycle of libusb_transfers.
    //
...
bool LibusbConnection::StartImpl(std::string* error) {
    if (!device_handle_) {
        *error = "device not opened";
        return false;
    }

    VLOG(USB) << "registered new usb device '" << serial() << "'";
...
static void process_device(libusb_device* device_raw) {
    ...
    VLOG(USB) << "constructed LibusbConnection for device " << connection->serial() << " ("
              << device_address << ")";

    register_usb_transport(connection, connection->serial().c_str(), device_address.c_str(), true);
}
...
static void device_disconnected(libusb_device* device) {
    ...
    if (it != usb_handles.end()) {
        ...
        fdevent_run_on_looper([connection_weak]() {
            ...
            if (connection) {
                connection->Stop();
                VLOG(USB) << "libusb_hotplug: device disconnected: " << connection->serial();
            } else {
