    ...
    while (true) {
        ...
        for (auto& port : ports) {
            ...
            VLOG(TRANSPORT) << "retry port " << port.port << ", last retry_count "
                            << port.retry_count;
            if (local_connect_arbitrary_ports(port.console, port.port, &dummy) == 0) {
...
struct EmulatorConnection : public FdConnection {
    ...
    ~EmulatorConnection() {
        ...
        port.console = (console_port_ != -1) ? console_port_ : local_port_ - 1;
...
static int getEmulatorConsolePort(std::string serial) {
    int console;
    return (sscanf(serial.c_str(), "emulator-%d", &console) == 1) ? console : -1;
}
