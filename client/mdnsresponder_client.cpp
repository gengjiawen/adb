  ...
  public:
    ...
    ResolvedService(
        ...
        DNSServiceErrorType ret = DNSServiceGetAddrInfo(
                &sdref_, 0, interface_index, kDNSServiceProtocol_IPv4, host_target_.data(),
                register_service_ip, reinterpret_cast<void*>(this));

        if (ret != kDNSServiceErr_NoError) {
            D("Got %d from DNSServiceGetAddrInfo.", ret);
        } else {
            D("DNSServiceGetAddrInfo(sdref=%p, host_target=%s)", sdref_, host_target_.data());
            Initialize();
        }
    ...
    bool ConnectSecureWifiDevice() {
        ...
        std::string response;
        connect_device(android::base::StringPrintf("%s.%s", service_name_.data(), reg_type_.data()),
                       &response);
        D("Secure connect to %s regtype %s (%s:%hu) : %s", service_name_.data(), reg_type_.data(),
          ip_addr_.data(), port_, response.data());
        return true;
    }
    ...
    static void AddToServiceRegistry(std::unique_ptr<ResolvedService> service) {
        ...
        if (adb_DNSServiceShouldAutoConnect(reg_type, service_name)) {
            ...
            D("Attempting to connect service_name=[%s], regtype=[%s] ip_addr=(%s:%hu)",
                    service_name.data(), reg_type.data(), ip_addr.data(), port);

            if (*service_index == kADBSecureConnectServiceRefIndex) {
                s->ConnectSecureWifiDevice();
            } else {
                connect_device(android::base::StringPrintf("%s.%s", service_name.data(),
                            reg_type.data()), &response);
                D("Connect to %s regtype %s (%s:%hu) : %s", service_name.data(), reg_type.data(),
                        ip_addr.data(), port, response.data());
            }
        } else {
    ...
    std::optional<int> service_index() const { return adb_DNSServiceIndexByName(reg_type_.data()); }
...
void ResolvedService::ForEachService(const ServiceRegistry& services,
    ...
    for (const auto& service : services) {
        ...
        if (wanted_service_name.empty()) {
            cb(service_name.data(), reg_type.data(), ip.data(), port);
...
bool ResolvedService::ConnectByServiceName(const ServiceRegistry& services,
    ...
    for (const auto& service : services) {
        ...
        if (wanted_name == service_name) {
            D("Got service_name match [%s]", wanted_name.data());
            return service->ConnectSecureWifiDevice();
        }
    }
    D("No registered service_names matched [%s]", service_name.data());
    return false;
}

// static
void ResolvedService::RemoveDNSService(const std::string& reg_type, const std::string& service_name) {
    D("%s: reg_type=[%s] service_name=[%s]", __func__, reg_type.data(), service_name.data());
    ...
    services->erase(std::remove_if(services->begin(), services->end(),
                                   [&sName](std::unique_ptr<ResolvedService>& service) {
                                       return (sName == service->service_name());
                                   }),
                    services->end());
}
...
void DNSSD_API on_service_browsed(DNSServiceRef sdref, DNSServiceFlags flags,
    ...
    if (flags & kDNSServiceFlagsAdd) {
        D("%s: Discover found new service_name=[%s] regtype=[%s] domain=[%s]", __func__, service_name,
          regtype, domain);
...
std::string mdns_list_discovered_services() {
    ...
    auto cb = [&](const std::string& service_name, const std::string& reg_type,
        ...
        result += android::base::StringPrintf("%s\t%s\t%s:%u\n", service_name.data(),
                reg_type.data(), ip_addr.data(), port);
    };
...
std::optional<MdnsInfo> mdns_get_connect_service_info(const std::string& name) {
    ...
    auto cb = [&](const std::string& service_name, const std::string& reg_type,
            const std::string& ip_addr, uint16_t port) {
        info.emplace(service_name, reg_type, ip_addr, port);
    };
