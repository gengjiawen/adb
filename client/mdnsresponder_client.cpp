...
class ResolvedRecord : public AsyncServiceRef {
  ...
  private:
    static void DNSSD_API DnsQueryRecordCallback(DNSServiceRef sdRef, DNSServiceFlags flags,
...
// static
void DNSSD_API ResolvedRecord::DnsQueryRecordCallback(
    ...
    std::unique_ptr<ResolvedRecord> data(static_cast<ResolvedRecord*>(context));
...
class ResolvedService : public AsyncServiceRef {
  ...
  private:
    void ReconfirmRecord() {
        bool expected = false;
        if (reconfirm_in_flight.compare_exchange_strong(expected, true)) {
            ...
            ResolvedRecord* resolved_record = new ResolvedRecord(
                    interface_index_, fullname, kDNSServiceType_SRV, kDNSServiceClass_IN,
                    [&](uint16_t rdlen, const void* rdata) {
                        if (rdlen > 0) {
                            std::string fullname = service_name_ + "." + reg_type_ + "local";
                            D("Got SRV record rdlen=%u", rdlen);
                            auto err = DNSServiceReconfirmRecord(
                                    0, interface_index_, fullname.c_str(), kDNSServiceType_SRV,
                                    kDNSServiceClass_IN, rdlen, rdata);
                            D("DNSServiceReconfirmRecord err=%d", err);
                        }
                        reconfirm_in_flight = false;
                    });
            if (!resolved_record->Initialized()) {
    ...
    std::atomic_bool reconfirm_in_flight{false};
};
...
void ResolvedService::ReconfirmRecordByServiceName(const ServiceRegistry& services,
    ...
    for (const auto& service : services) {
        ...
        if (wanted_name == service_name) {
            D("Got service_name match [%s]", wanted_name.c_str());
            fdevent_run_on_main_thread([&]() { service->ReconfirmRecord(); });
            return;
        }
    }
    D("No registered service_names matched [%s]", service_name.c_str());
