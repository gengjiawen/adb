        ...
        switch (local_endpoint_.address.version()) {
            ...
            case UdpSocket::Version::kV6: {
                ...
                if (adb_getsockname(fd_, reinterpret_cast<sockaddr*>(&sa), &sa_len) != -1) {
                    local_endpoint_.address = GetIPAddressFromSockAddr(sa);
                    local_endpoint_.port = GetPortFromFromSockAddr(sa);
                    LOG(INFO) << "bind endpoint=" << local_endpoint_ << " scope_id=" << sa.sin6_scope_id;
                }
                return;
            }
            default:
...
//                // Appropriate address is set based on |imr_ifindex| when set.
                ...
                if (getsockopt(fd_.get(), IPPROTO_IP, IP_MULTICAST_IF, &default_addr, &buf_size) != -1) {
                    auto default_ip = IPAddress(IPAddress::Version::kV4,
        ...
        switch (local_endpoint_.address.version()) {
            case UdpSocket::Version::kV4: {
                ...
                local_endpoint_.address.CopyToV4(
                        reinterpret_cast<uint8_t*>(&multicast_properties.imr_address.s_addr));
                multicast_properties.imr_ifindex = static_cast<IPv4NetworkInterfaceIndex>(ifindex);
                static_assert(sizeof(multicast_properties.imr_multiaddr) == 4u,
                              "IPv4 address requires exactly 4 bytes");
                address.CopyToV4(reinterpret_cast<uint8_t*>(&multicast_properties.imr_multiaddr.s_addr));
                LOG(INFO) << "IP_ADD_MEMBERSHIP addr=" << address << " ifindex=" << multicast_properties.imr_ifindex;
#endif  // _WIN32
...
#ifdef _WIN32
                // TODO: OSP-discovery uses INADDR_ANY here, which doesn't work with WSASendMsg.
                // So let's just explicitly use the link-local multicast address for now.
                discovery::kDefaultMulticastGroupIPv4.CopyToV4(
                        reinterpret_cast<uint8_t*>(&sa.sin_addr.s_addr));
#else
                // TODO: OSP-discovery uses INADDR_ANY here, which doesn't seem to resolve to the
                // correct interface if the socket is bound to INADDR_ANY. The observed behavior is
                // both the source and destination address resolves to the loopback address on
                // linux, but we expect the source address to be the specified interface address and
                // the destination address to be the multicast address.
                discovery::kDefaultMulticastGroupIPv4.CopyToV4(
                        reinterpret_cast<uint8_t*>(&sa.sin_addr.s_addr));
#endif
                msg.msg_name = &sa;
