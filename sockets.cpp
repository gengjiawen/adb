...
static SocketFlushResult local_socket_flush_incoming(asocket* s) {
    D("LS(%u) local_socket_flush_incoming: %zu bytes in queue", s->id, s->packet_queue.size());
    ...
    if (!s->packet_queue.empty()) {
        ...
        D("LS(%u) local_socket_flush_incoming: rc = %zd", s->id, rc);
        if (rc > 0) {
            bytes_flushed = rc;
...
void connect_to_remote(asocket* s, std::string_view destination) {
    ...
    if (s->transport->SupportsDelayedAck()) {
        p->msg.arg1 = MAX_DELAYED_ACK_BYTES;
        s->available_send_bytes = MAX_DELAYED_ACK_BYTES;
    }
