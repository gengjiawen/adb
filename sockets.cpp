        ...
        if (r > 0) {
            if (available_send_bytes) {
                if (*available_send_bytes <= 0) {
                    D("LS(%u): send buffer full (%" PRId64 ")", saved_id, *available_send_bytes);
    ...
    */
    if (ev & FDE_WRITE) {
        switch (s->Flush_incoming()) {
            case LocalSocket::SocketFlushResult::Destroyed:
                return;

            case LocalSocket::SocketFlushResult::TryAgain:
                break;

            case LocalSocket::SocketFlushResult::Completed:
                break;
        }
    }
...
RemoteSocket* create_remote_socket(unsigned id, atransport* t) {
    if (id == 0) {
...
int SmartSocket::enqueue(apacket::payload_type data) {
    std::string_view service;
    std::string_view serial;
    TransportId transport_id = 0;
    TransportType type = kTransportAny;

    D("SS(%d): enqueue %zu", id, data.size());

    if (smart_socket_data.empty()) {
        // TODO: Make this an IOVector?
        smart_socket_data.assign(data.begin(), data.end());
    } else {
        std::copy(data.begin(), data.end(), std::back_inserter(smart_socket_data));
    }

    /* don't bother if we can't decode the length */
    if (smart_socket_data.size() < 4) {
        return 0;
    }

    uint32_t len = unhex(smart_socket_data.data(), 4);
    if (len == 0 || len > MAX_PAYLOAD) {
        D("SS(%d): bad size (%u)", id, len);
        goto fail;
    }

    D("SS(%d): len is %u", id, len);
    /* can't do anything until we have the full header */
    if ((len + 4) > smart_socket_data.size()) {
        D("SS(%d): waiting for %zu more bytes", id, len + 4 - smart_socket_data.size());
        return 0;
    }

    smart_socket_data[len + 4] = 0;

    D("SS(%d): '%s'", id, (char*)(smart_socket_data.data() + 4));

    service = std::string_view(smart_socket_data).substr(4);
    ...
    if (android::base::ConsumePrefix(&service, "host-serial:")) {
        ...
        if (!internal::parse_host_service(&serial, &service, service)) {
            LOG(ERROR) << "SS(" << id << "): failed to parse host service: " << service;
            goto fail;
        }
    } else if (android::base::ConsumePrefix(&service, "host-transport-id:")) {
        if (!ParseUint(&transport_id, service, &service)) {
            LOG(ERROR) << "SS(" << id << "): failed to parse host transport id: " << service;
            return -1;
        }
        if (!android::base::ConsumePrefix(&service, ":")) {
            LOG(ERROR) << "SS(" << id << "): host-transport-id without command";
            return -1;
        }
    ...
    if (!service.empty()) {
        ...
        switch (host_request_result) {
            case HostRequestResult::Handled:
                LOG(VERBOSE) << "SS(" << id << "): handled host service '" << service << "'";
                goto fail;

            case HostRequestResult::SwitchedTransport:
                D("SS(%d): okay transport", id);
                smart_socket_data.clear();
                return 0;
        ...
        */
        ...
        if (s2 == nullptr) {
            LOG(VERBOSE) << "SS(" << id << "): couldn't create host service '" << service << "'";
            SendFail(peer->fd, "unknown host service");
            goto fail;
        }
    ...
    if (!transport) {
        SendFail(peer->fd, "device offline (no transport)");
        goto fail;
    } else if (!ConnectionStateIsOnline(transport->GetConnectionState())) {
        /* if there's no remote we fail the connection
         ** right here and terminate it
         */
        SendFail(peer->fd, "device offline (transport offline)");
        goto fail;
    }
    ...
    */
    // FAB
    //    peer->ready = local_socket_ready_notify;
    //    peer->close = local_socket_close_notify;

    /* give them our transport and upref it */
    peer->transport = transport;

    connect_to_remote(peer, std::string_view(smart_socket_data).substr(4));
    peer = nullptr;
    close();
    return 1;
...
static SmartSocket* create_smart_socket(void) {
...
void connect_to_smartsocket(asocket* s) {
    ...
    SmartSocket* ss = create_smart_socket();
    s->peer = ss;
    ss->peer = s;
    s->ready();
}
